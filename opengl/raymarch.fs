...
vec3 lightPoint(in vec3 p) {
  ...
  mat4 testMtx = inverse(transpose(viewMatrix));
...
void main() {
  ...
  vec4 d = vec4(left + gl_FragCoord.x * hstep, bottom + gl_FragCoord.y * vstep, focal, 1);
  //d = viewMatrix * d;
  float t, lum;

  //d = inverse(transpose(viewMatrix)) * d;
  //trace((viewMatrix * vec4(0,0,0,1)).xyz, d.xyz, t, lum);
  trace(vec3(0,0.89,1.78), d.xyz, t, lum);
  //trace(vec3(0,0,0), d.xyz, t, lum);
  if(lum > 0) {
    fragColor = vec3(lightPoint(t*d.xyz));
  } else {
